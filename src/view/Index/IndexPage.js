...
class IndexPage extends React.PureComponent {
    ...
    // 获取数据
    getData = () => {
        const {onLoadBannerData, onLoadWeatherData, onLoadTopPlayListHigh, onLoadRecommendData} = this.props;
        ...
        return (
            <Animated.View style={styles.bannerBox}>
                <Swiper paginationStyle={styles.paginationStyle} autoplay={true}>
                    {banner.map((item) => {
                        return <Image style={styles.bannerImage} key={item.id} source={{uri: item.imageUrl}} />;
                    })}
                </Swiper>
            </Animated.View>
        );
    ...
    renderGuessLike = () => {
        ...
        return (
            <View style={styles.guessLikeBox}>
