    ...
    _rederRadioBanner = () => {
        ...
        return (
            <View style={styles.bannerBox}>
                <Swiper paginationStyle={styles.paginationStyle} autoplay={true}>
                    {radioBanner &&
                        radioBanner.map((item, index) => {
                            return <Image style={styles.bannerImage} key={index} source={{uri: item.pic}} />;
                        })}
                </Swiper>
            </View>
        );
                    ...
                    menu_data.map((item) => {
                        return (
                            <TouchableOpacity
                                ...
                                <Text style={{marginTop: px2dp(6)}} key={item.id}>
                                    {item.title}
                                </Text>
                            </TouchableOpacity>
                        );
        ...
        return (
            ...
            <Text>换一换</Text>
                ...
                <ScrollView showsHorizontalScrollIndicator={false} horizontal={true}>
                    {radioRaking &&
                        radioRaking.map((item) => {
                            return (
                                <TouchableOpacity
                                    onPress={() => this.goToMorePage(item.id)}
                                    activeOpacity={1}
                                    style={styles.rakingItemBox}
                                    key={item.id}>
...
export default connect(
    ({radio, radioBanner, radioRaking}) => ({radio, radioBanner, radioRaking}),
    (dispatch) => ({
        onLoadRadioData(url) {
